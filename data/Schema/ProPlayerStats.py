...
##exact same table as PlayerDails but inheritance would lead to a relationship by sqlalchemy's standards
class ProPlayerDetail(Base, object):
    __tablename__ = "proplayerdetails"
    ...
    def __init__(self, dict: dict):
        ...
        val = dict.get("bans", [])
        l = len(val)
        if l > 0: self.ban0 = val[0].get("championId", 0)
        if l > 1: self.ban1 = val[1].get("championId", 0)
        if l > 2: self.ban2 = val[2].get("championId", 0)
        if l > 3: self.ban3 = val[3].get("championId", 0)
        if l > 4: self.ban4 = val[4].get("championId", 0)
