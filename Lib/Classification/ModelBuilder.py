...
def deep_neural_network_classifier(num_feat, classes, layers, units_per_layer, dropout, activation='relu'):
    model = Sequential()
    if type(dropout) is float:
        dropout = np.repeat(dropout, layers, axis=0)
    if type(units_per_layer) is int:
        units_per_layer = np.repeat(units_per_layer, layers, axis=0)
        for layer in range(layers):
            model.add(Dense(units_per_layer[layer], input_dim=num_feat, activation=activation))
