...
class Classifier:
    ...
    def run_naive_bayes(self, data):
        ...
        print("Train/Test Accuracy: " + str(train_acc) + '/' + str(test_acc))
    ...
    def run_neural_network(self, data):
        ...
        clf = MLPClassifier(activation='relu', alpha=0.0001, batch_size='auto', beta_1=0.9,
                            ...
                            shuffle=True, solver='adam', tol=0.0001, validation_fraction=0.33,
    ...
    def run_deep_learning_1(self, data):
        ...
        history = model1.fit(x_train, y_train, validation_split=0.33, callbacks=[monitor, checkpointer], verbose=0,
                             epochs=1000).history
        plot_accuracy_dev(history['acc'], history['val_acc'], path)
        plot_loss_dev(history['loss'], history['val_loss'], path)
        pred = model1.predict(x_test)
        pred = np.argmax(pred, axis=1)
        y_test2 = np.argmax(y_test, axis=1)
        ...
        #     # Fit the model
        #     model.fit(x[train], y[train], epochs=150, validation_split=0.33,
        #               batch_size=10, verbose=0, callbacks=[monitor, checkpointer])
        #     # evaluate the model
    ...
    def run_deep_learning_3(self, data):
        ...
        start = datetime.now()
        path = self.setup(name)
        model1, monitor, checkpointer = build_model3(x, y, path)
        history = model1.fit(x_train, y_train, validation_split=0.33, callbacks=[monitor, checkpointer], verbose=0,
                             epochs=1000).history
        plot_accuracy_dev(history['acc'], history['val_acc'], path)
        plot_loss_dev(history['loss'], history['val_loss'], path)
    ...
    def run_deep_learning_2(self, data):
        ...
        path = self.setup(name)
        model2, monitor, checkpointer = build_model2(x, y, path)
        history = model2.fit(x_train.reshape(x_train.shape[0], x_train.shape[1], 1), y_train, epochs=2,
                             validation_split=0.33, callbacks=[monitor, checkpointer], verbose=0).history
        plot_accuracy_dev(history['acc'], path)
        plot_loss_dev(history['loss'], path)
        pred = model2.predict(x_test.reshape(x_test.shape[0], x_test.shape[1], 1))
        pred = np.argmax(pred, axis=1)
        y_test2 = np.argmax(y_test, axis=1)
        precision = accuracy_score(y_test2, pred)
        ...
        for train, test in kfold.split(x, y):
            # create model
            model, monitor, checkpointer = build_model2(x, y, path)
            # Fit the model
            model.fit(x[train].reshape(x[train].shape[0], x[train].shape[1], 1), y[train], epochs=150,
                      validation_split=0.1, callbacks=[monitor, checkpointer], batch_size=10, verbose=0)
            # evaluate the model
            scores = model.evaluate(x[test].reshape(x[test].shape[0], x[test].shape[1], 1), y[test], verbose=0)
            print("%s: %.2f%% , %s: %.2f%%" % (
            model.metrics_names[1], scores[1] * 100, model.metrics_names[0], scores[0]))
            cvscores.append(scores[1] * 100)
        print("%.2f%% (+/- %.2f%%)" % (np.mean(cvscores), np.std(cvscores)))
        return preds
