...
df = dh.get_purchase_teams(champions=set_1, patches=PATCHES, tiers=["CHALLENGER", "MASTER", "DIAMOND", "PLATINUM"],
...
df = dh.one_hot_encode_columns(df, ['side', 'tier', 'masteryId', 'championId', 'type', 'platformId'], drop=False)
feat = df.columns.difference(
    ['_id', 'gameId', 'participantId', 'platformId', 'tier', 'type', 'patch', 'masteryId', 'championId',
     'itemId', 'itemId_fact'])
...
x, y = to_xy(df, feat, length=15)
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.1)

# for 1 Layer:
for param in [0.5, 1, 1.5, 2, 2.5, 3]:
    print("Try with Factor: ", str(param))
    model, monitor, checkpointer = build_param_model(x, y, setup(name="LSTM_" + str(param)), param)
...
model = mb.lstm_classifier(num_feat=x.shape[2], classes=y.shape[2], timesteps=x.shape[1],
                           batch_size=5, units_per_layer=[128, 64], layers=2, dropout=0.3,
                           optimizer='adam')
model.summary()
history = model.fit(x_train, y_train, epochs=1000, validation_split=0.1, callbacks=[monitor, checkpointer],
...
# model, monitor, checkpointer = build_param_model_2a(x, y, path, units=units, dropout=0.3)
model = mb.lstm_classifier(num_feat=x.shape[2], classes=y.shape[2], timesteps=x.shape[1],
                           batch_size=5, units_per_layer=[128, 128], layers=2, dropout=0.3,
                           optimizer='adam')
model.summary()
history = model.fit(x_train, y_train, epochs=1000, validation_split=0.1, callbacks=[monitor, checkpointer],
                    batch_size=5, verbose=False, shuffle=False).history
...
# model, monitor, checkpointer = build_param_model_2a(x, y, path, units=units, dropout=0.3)
model = mb.lstm_classifier(num_feat=x.shape[2], classes=y.shape[2], timesteps=x.shape[1],
                           batch_size=5, units_per_layer=[64, 64, 64, 64], layers=4, dropout=0.3,
                           optimizer='adam')
model.summary()
history = model.fit(x_train, y_train, epochs=1000, validation_split=0.1, callbacks=[monitor, checkpointer],
                    batch_size=5, verbose=False, shuffle=False).history
